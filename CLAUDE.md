...
And `CODEX_TUI_DISABLE_KEYBOARD_ENHANCEMENT=1` is set on a codex session's tmux env (`agent_cmd::env`). Codex negotiates the kitty keyboard protocol at startup; inside tmux that negotiation outlives the client, so every re-attach — which is what switching workspaces does — replays the queries to xterm.js, whose replies reach a codex no longer waiting for them and land in the composer as literal text (`0;276;0c`, the tail of a secondary-DA reply).
